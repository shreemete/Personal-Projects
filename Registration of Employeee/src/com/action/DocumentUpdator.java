...
public class DocumentUpdator extends HttpServlet {
	/*
	 *
	 *
	 *
...
//			System.out.println(id);
			String fileSavePath =Path.path+Delete.path_id+"\\";
			Part _part = null;         
			...
			Iterate it = new Iterate(Delete.path_id,arr);
			boolean f;
			try {
				
				//System.out.print(Delete.path_id+"======>");
				f = UpdateDao.updateStudentToDB(it);
				if(f) {
					
					out.println("<script>alert('Data Added Successfully...')</script>");

					response.sendRedirect("updateDocument.jsp?id"+Delete.path_id);
				}else {
